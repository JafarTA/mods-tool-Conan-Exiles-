...
LANG_DICT = {
    "zh-TW": {
        "title": "流放者柯南 終極防衝突模組管理器 (V9.4.1 簡繁雙語智能面板版)",
        ...
        "success": "成功",
        "error": "錯誤",
        "save_success": "模組清單儲存成功！帶有 -windows 後綴的殘留模組已自動被徹底清洗抹除！",
    ...
    "zh-CN": {
        "title": "流放者柯南 终极防冲突模组管理器 (V9.4.1 简繁双语智能面板版)",
        ...
        "success": "成功",
        "error": "错误",
        "save_success": "模组清单保存成功！带有 -windows 后缀的残留模组已被彻底清洗抹除！",
...
class SaveBackupDialog(QDialog):
    ...
    def init_ui(self):
        ...
        self.lbl_path = QLabel("存檔路徑 (Saved): 未指定" if self.lang_code == "zh-TW" else "存档路径 (Saved): 未指定")
        self.lbl_path.setWordWrap(True)
        self.btn_browse_save = QPushButton("📂 變更存檔路徑" if self.lang_code == "zh-TW" else "📂 变更存档路径")
        ...
        self.btn_do_backup = QPushButton("📸 建立全新時間戳記備份" if self.lang_code == "zh-TW" else "📸 创建全新时间戳备份")
        self.btn_do_backup.setStyleSheet("background-color: #34495e; color: white; font-weight: bold; padding: 6px; border: none;")
        self.btn_do_backup.clicked.connect(self.create_backup)
        
        self.btn_open_backup_dir = QPushButton("📂 開啟備份資料夾" if self.lang_code == "zh-TW" else "📂 打开备份文件夹")
        ...
        self.btn_restore = QPushButton("⏪ 覆蓋並還原選中存檔" if self.lang_code == "zh-TW" else "⏪ 覆盖并还原选中存档")
        self.btn_restore.setStyleSheet("background-color: #922b21; color: white; font-weight: bold; border: none;")
        self.btn_restore.clicked.connect(lambda: self.restore_backup(self.list_backups.currentItem()))
        
        self.btn_delete = QPushButton("❌ 刪除此備份" if self.lang_code == "zh-TW" else "❌ 删除此备份")
    ...
    def auto_detect_save_path(self):
        if self.game_exe and os.path.exists(self.game_exe) and not self.game_exe.startswith("steam://"):
            ...
            if os.path.exists(root_dir):
                ...
                self.lbl_path.setText(f"偵測到存檔路徑: {self.save_dir_path}" if self.lang_code == "zh-TW" else f"侦测到存档路径: {self.save_dir_path}")
                return
            ...
            try:
                with open(cache_path, "r", encoding="utf-8") as f:
                    ...
                    if "last_save_dir" in cfg and os.path.exists(cfg["last_save_dir"]):
                        ...
                        self.lbl_path.setText(f"已讀取快取路徑: {self.save_dir_path}" if self.lang_code == "zh-TW" else f"已读取缓存路径: {self.save_dir_path}")
                        return
            except Exception:
                pass
        self.lbl_path.setText("❌ 未偵測到存檔路徑，請手動指定遊戲 Saved 資料夾！" if self.lang_code == "zh-TW" else "❌ 未侦测到存档路径，请手动指定游戏 Saved 文件夹！")

    def browse_save_dir(self):
        title = "選擇流放者柯南的 Saved 資料夾" if self.lang_code == "zh-TW" else "选择流放者柯南的 Saved 文件夹"
        dir_path = QFileDialog.getExistingDirectory(self, title, self.save_dir_path)
        if dir_path:
            self.save_dir_path = dir_path
            self.lbl_path.setText(f"手動路徑: {self.save_dir_path}" if self.lang_code == "zh-TW" else f"手动路径: {self.save_dir_path}")
    ...
    def refresh_backup_list(self):
        ...
        for d in dirs:
            prefix = "📦 備份排程：" if self.lang_code == "zh-TW" else "📦 备份排程："
    ...
    def create_backup(self):
        if not self.save_dir_path or not os.path.exists(self.save_dir_path):
            msg = "請先設定正確的 Saved 存檔資料夾路徑！" if self.lang_code == "zh-TW" else "请先设置正确的 Saved 存档文件夹路径！"
            QMessageBox.warning(self, "錯誤" if self.lang_code == "zh-TW" else "错误", msg)
            return
        
        title = "新增存檔備份" if self.lang_code == "zh-TW" else "新增存档备份"
        prompt = "請輸入此備份的備忘備註 (可留空):" if self.lang_code == "zh-TW" else "请输入此备份的备忘备注 (可留空):"
        ...
        try:
            ...
            success_msg = f"遊戲存檔已安全備份至：\n{folder_name}" if self.lang_code == "zh-TW" else f"游戏存档已安全备份至：\n{folder_name}"
            QMessageBox.information(self, "備份成功" if self.lang_code == "zh-TW" else "备份成功", success_msg)
            self.refresh_backup_list()
        except Exception as e:
            QMessageBox.critical(self, "備份失敗" if self.lang_code == "zh-TW" else "备份失败", f"錯誤：\n{str(e)}")

    def restore_backup(self, item):
        if not item: return
        if not self.save_dir_path or not os.path.exists(self.save_dir_path):
            msg = "找不到原遊戲存檔 Saved 資料夾，無法執行覆蓋還原！" if self.lang_code == "zh-TW" else "找不到原游戏存档 Saved 文件夹，无法执行覆盖还原！"
            QMessageBox.warning(self, "錯誤" if self.lang_code == "zh-TW" else "错误", msg)
            return
        ...
        title = "⚠️ 強制覆蓋警告" if self.lang_code == "zh-TW" else "⚠️ 强制覆盖警告"
        confirm_msg = f"您確定要將目前的遊戲存檔【完全覆蓋還原】為以下備份嗎？\n\n【{b_folder}】\n\n※此操作不可逆，目前的進度將被完全蓋除！" if self.lang_code == "zh-TW" else f"您确定要将当前的游戏存档【完全覆盖还原】为以下备份吗？\n\n【{b_folder}】\n\n※此操作不可逆，当前的进度将被完全盖除！"
        
        confirm = QMessageBox.question(
            self, title, confirm_msg,
            ...
            try:
                for item_name in os.listdir(source_backup_dir):
                    ...
                    if os.path.isdir(s):
                        ...
                        shutil.copy2(s, d)
                success_title = "還原成功" if self.lang_code == "zh-TW" else "还原成功"
                success_text = "存檔覆蓋還原成功！請重新啟動遊戲確認。" if self.lang_code == "zh-TW" else "存档覆盖还原成功！请重新启动游戏确认。"
                QMessageBox.information(self, success_title, success_text)
            except Exception as e:
                QMessageBox.critical(self, "還原失敗" if self.lang_code == "zh-TW" else "还原失败", f"衝突錯誤：\n{str(e)}")
    ...
    def delete_backup(self):
        ...
        title = "刪除確認" if self.lang_code == "zh-TW" else "删除确认"
        msg = f"確定要徹底刪除此備份檔案嗎？\n{b_folder}" if self.lang_code == "zh-TW" else f"确定要彻底删除此备份文件吗？\n{b_folder}"
        ...
        if confirm == QMessageBox.StandardButton.Yes:
            try:
                ...
                QMessageBox.critical(self, "錯誤" if self.lang_code == "zh-TW" else "错误", f"刪除失敗：{str(e)}")
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.selected_tag_color = "#3a3a4a" 
        self.active_threads = []
    ...
    def init_ui(self):
        ...
        self.btn_ui_toggle.setStyleSheet("background-color: #614e1a; color: #fff; font-weight: bold; border-color: #7d6608;")
        ...
        self.btn_set_exe.setStyleSheet("background-color: #424d57; color: white;")
        self.btn_set_exe.clicked.connect(self.set_custom_game_exe)

        self.btn_archive_mgr = QPushButton(self.strings["archive_btn"])
        self.btn_archive_mgr.setStyleSheet("background-color: #1a5276; color: white; font-weight: bold;")
        self.btn_archive_mgr.clicked.connect(self.open_save_backup_manager)

        self.btn_save = QPushButton(self.strings["save_btn"])
        self.btn_save.setStyleSheet("background-color: #21618c; color: white; font-weight: bold;")
        self.btn_save.clicked.connect(self.save_modlist)
        
        self.btn_launch = QPushButton(self.strings["launch_btn"])
        self.btn_launch.setStyleSheet("background-color: #196f3d; color: white; font-weight: bold; border-color: #1e8449;")
        ...
        self.lbl_mod_op_sep = QLabel("--- 模組標籤操作 ---" if self.current_lang_code == "zh-TW" else "--- 模组标签操作 ---")
        ...
        self.lbl_mod_img = QLabel("【無預覽圖】" if self.current_lang_code == "zh-TW" else "【无预览图】")
        self.lbl_mod_img.setAlignment(Qt.AlignmentFlag.AlignCenter)
        self.lbl_mod_img.setFixedSize(220, 220)
        self.lbl_mod_img.setStyleSheet("border: 1px solid #3a3a4a; background-color: #111; border-radius: 6px;")
        ...
        self.lbl_prot_title = QLabel("🛡️ 模組安全狀態防護設定 :" if self.current_lang_code == "zh-TW" else "🛡️ 模组安全状态防护设置 :")
        self.lbl_prot_title.setStyleSheet("font-weight: bold; color: #ec7063;")
        prot_layout.addWidget(self.lbl_prot_title)
        
        btn_layout = QHBoxLayout()
        self.btn_panel_lock = QPushButton("🔒 切換 鎖定狀態" if self.current_lang_code == "zh-TW" else "🔒 切换 锁定状态")
        self.btn_panel_lock.setStyleSheet("background-color: #5b2c6f; color: white; font-weight: bold;")
        self.btn_panel_lock.clicked.connect(self.toggle_current_mod_lock)
        
        self.btn_panel_mutex = QPushButton("⚔️ 設定 互斥群組" if self.current_lang_code == "zh-TW" else "⚔️ 设置 互斥群组")
        self.btn_panel_mutex.setStyleSheet("background-color: #b7950b; color: white; font-weight: bold;")
    ...
    def toggle_ui_language(self):
        ...
        self.lbl_mod_op_sep.setText("--- 模組標籤操作 ---" if self.current_lang_code == "zh-TW" else "--- 模组标签操作 ---")
        self.lbl_prot_title.setText("🛡️ 模組安全狀態防護設定 :" if self.current_lang_code == "zh-TW" else "🛡️ 模组安全状态防护设置 :")
    ...
    # 🚀【核心優化】：翻譯功能完全適應簡體中文與繁體中文雙軌制，並快取分離
    def get_smart_translation(self, text, clean_path, target_lang):
        ...
        try:
            ...
            # 依據目前的目標語系，動態向 API 請求對應代碼
            api_target = "zh-TW" if target_lang == "zh-TW" else "zh-CN"
            params = {"client": "gtx", "sl": "auto", "tl": api_target, "dt": "t", "q": text}
        ...
        tw_style = {"模组":"模組","数据":"資料","文件":"檔案","说明":"說明","未知":"未知","无":"無"}
    ...
    def set_custom_game_exe(self):
        title = "選擇遊戲主程式執行檔" if self.current_lang_code == "zh-TW" else "选择游戏主程序执行档"
        file_path, _ = QFileDialog.getOpenFileName(self, title, "", "Executable Files (*.exe);;All Files (*)")
        if file_path:
            self.game_exe_path = file_path
            self.save_global_tags()
            QMessageBox.information(self, self.strings["success"], f"遊戲啟動路徑已設定為:\n{file_path}" if self.current_lang_code == "zh-TW" else f"游戏启动路径已设置为:\n{file_path}")

    def select_modlist_file(self):
        title = "選擇 modlist.txt 或 Mods 資料夾" if self.current_lang_code == "zh-TW" else "选择 modlist.txt 或 Mods 文件夹"
        file_path, _ = QFileDialog.getOpenFileName(self, title, "", "Conan Modlist (modlist.txt);;All Files (*)")
        if file_path:
    ...
    def clean_ghost_windows_paths(self, raw_dict):
        cleaned = {}
        if not isinstance(raw_dict, dict): return cleaned
        for k, v in raw_dict.items():
            if not isinstance(k, str): continue
            if "-windows" in k.lower() or "_windows" in k.lower():
                continue
            cleaned[k] = v
        return cleaned
    ...
    def load_modlist(self):
        ...
        if os.path.exists(db_path):
            try:
                with open(db_path, "r", encoding="utf-8") as f:
                    ...
                    cleaned_db = self.clean_ghost_windows_paths(loaded_data)
                    self.mod_data_map.update(cleaned_db)
            except Exception:
                pass
        ...
        try:
            ...
            for line in lines:
                ...
                if "-windows" in raw_path.lower() or "_windows" in raw_path.lower():
                    continue
                ...
                valid_modlist_paths.add(final_path_key)
            ...
            purged_map = {}
            for k, v in self.mod_data_map.items():
                if k in valid_modlist_paths:
                    purged_map[k] = v
            self.mod_data_map = purged_map
    ...
    def update_item_display_style(self, item, clean_path, filename, row_idx=None):
        ...
        if custom_color:
            ...
            processed_color = QColor.fromHsv(h, s, v)
            item.setBackground(QBrush(processed_color))
            
            if (processed_color.red()*0.299 + processed_color.green()*0.587 + processed_color.blue()*0.114) < 125:
                item.setForeground(QBrush(QColor("#ffffff")))
            else:
                item.setForeground(QBrush(QColor("#111116")))
        else:
        ...
        if tag: display_text = f"[{row_idx:02d}] 🏷️{tag} | {filename}"
        if mutex: display_text = f"{display_text} <⚔️群組: {mutex}>" if self.current_lang_code == "zh-TW" else f"{display_text} <⚔️群组: {mutex}>"
    ...
    def on_item_checked_changed(self, item):
        ...
        if mdata.get("is_locked", False) and item.checkState() == Qt.CheckState.Unchecked:
            ...
            item.setCheckState(Qt.CheckState.Checked)
            self.mod_list_widget.blockSignals(False)
            msg = "該模組已被鎖定保護，無法變更為不啟用！" if self.current_lang_code == "zh-TW" else "该模组已被锁定保护，无法变更为不启用！"
            QMessageBox.warning(self, "保護中" if self.current_lang_code == "zh-TW" else "保护中", msg)
            return
    ...
    def set_current_mod_mutex(self):
        ...
        title = "設定互斥群組" if self.current_lang_code == "zh-TW" else "设置互斥群组"
        prompt = f"目前群組: {current_group if current_group else '無'}\n請輸入群組代號（留空則代表清除互斥）:" if self.current_lang_code == "zh-TW" else f"当前群组: {current_group if current_group else '无'}\n请输入群组代号（留空则代表清除互斥）:"
        
        group_name, ok = QInputDialog.getText(
            self, title, prompt, QLineEdit.EchoMode.Normal, current_group
        )
    ...
    def save_modlist(self):
        ...
        try:
            ...
            self.mod_data_map = self.clean_ghost_windows_paths(self.mod_data_map)
    ...
    def launch_conan_exiles(self):
        ...
        try:
            ...
            msg = "無法啟動遊戲，請重新檢查設定路徑！" if self.current_lang_code == "zh-TW" else "无法启动游戏，请重新检查设置路径！"
            QMessageBox.critical(self, self.strings["error"], f"{msg}\n錯誤: {str(e)}")
    ...
    def move_mod(self, steps):
        ...
        if 0 <= target_row < self.mod_list_widget.count():
            ...
            if self.mod_data_map.get(path_curr, {}).get("is_locked") or self.mod_data_map.get(path_targ, {}).get("is_locked"):
                msg = "偵測到移動路徑上存在【鎖定保護】的模組，已自動拒絕排序變更！" if self.current_lang_code == "zh-TW" else "侦测到移动路径上存在【锁定保护】的模组，已自动拒绝排序变更！"
                QMessageBox.warning(self, "禁止更動排序" if self.current_lang_code == "zh-TW" else "禁止更动排序", msg)
                return
                ...
                with open(old_config_path, "r", encoding="utf-8") as f:
                    ...
                    for k, v in old_data.items():
                        if k != "global_settings" and isinstance(v, dict):
                            if "-windows" not in k.lower() and "_windows" not in k.lower():
    ...
    def pick_library_tag_color(self):
        title = "選擇標籤顏色" if self.current_lang_code == "zh-TW" else "选择标签颜色"
        color = QColorDialog.getColor(QColor(self.selected_tag_color), self, title)
        if color.isValid():
            self.selected_tag_color = color.name()
            self.btn_pick_color.setStyleSheet(f"background-color: {self.selected_tag_color}; border: 1px solid #fff;")
    ...
    def on_library_tag_selected(self, current, previous):
        if current:
            ...
            self.selected_tag_color = self.global_tag_library.get(tag_text, "#3a3a4a")
    ...
    def apply_tag_by_name(self, row_idx, tag_text):
        color_hex = self.global_tag_library.get(tag_text, "#3a3a4a")
    ...
    def on_mod_selection_changed(self, row):
        ...
        if self.current_lang_code == "zh-TW":
            ...
            self.btn_panel_lock.setText("🔓 解除锁定保护" if is_locked else "🔒 开启锁定保护")
            self.btn_panel_mutex.setText(f"⚔️ 互斥: {mutex_group}" if mutex_group else "⚔️ 设置互斥群组")

        raw_description = mdata.get('description_zh_zh-CN', '無詳細說明')
        cleaned_original = self.clean_html_tags(raw_description)
        # 🚀 根據目前語系，智能獲取對應的繁體或簡體說明
        smart_translation = self.get_smart_translation(cleaned_original, clean_path, self.current_lang_code)

        header_lang = {
            "zh-TW": {"curr_id": "開關序號", "status": "安全狀態", "mutex": "互斥群組", "path": "完整路徑", "tag": "自訂標籤", "title": "工作坊標題", "update": "最後更新", "trans": "翻譯說明 (Traditional Chinese)", "orig": "工作坊英文原文 (English Original)"},
            "zh-CN": {"curr_id": "开关序号", "status": "安全状态", "mutex": "互斥群组", "path": "完整路径", "tag": "自订标签", "title": "创意工坊标题", "update": "最后更新", "trans": "翻译说明 (Simplified Chinese)", "orig": "创意工坊英文原文 (English Original)"}
        }[self.current_lang_code]

        # 🚀【細節優化】：詳情面板內的富文本排版也全面與語系聯動
        html_details = f"""
        <html>
        <head>
        <style>
            body {{ font-family: 'Segoe UI', 'Microsoft JhengHei', 'Microsoft YaHei', sans-serif; background-color: #141418; color: #d0d0d6; margin: 10px; line-height: 1.6; }}
            .metadata-table {{ width: 100%; border-collapse: collapse; margin-bottom: 15px; background-color: #1c1c22; border-radius: 6px; overflow: hidden; }}
            .metadata-table td {{ padding: 8px 12px; border-bottom: 1px solid #2a2a35; font-size: 13px; }}
            .metadata-table td.label {{ color: #888899; width: 25%; font-weight: bold; }}
            .metadata-table td.value {{ color: #ffffff; }}
            .badge-lock {{ background-color: #7b2cbf; color: white; padding: 2px 6px; border-radius: 4px; font-size: 11px; font-weight: bold; }}
            .badge-normal {{ background-color: #27ae60; color: white; padding: 2px 6px; border-radius: 4px; font-size: 11px; font-weight: bold; }}
            .section-title {{ font-size: 15px; font-weight: bold; color: #5dade2; border-left: 4px solid #3498db; padding-left: 8px; margin-top: 20px; margin-bottom: 10px; }}
            .section-title.orig {{ color: #eb984e; border-left-color: #e67e22; }}
            .content-box {{ background-color: #181820; border: 1px solid #2d2d38; border-radius: 6px; padding: 12px; font-size: 14px; white-space: pre-wrap; word-wrap: break-word; color: #e5e8e8; }}
            .content-box.english {{ color: #b2babb; font-size: 13px; }}
        </style>
        </head>
        <body>
            <table class="metadata-table">
                <tr>
                    <td class="label">{header_lang['curr_id']}</td>
                    <td class="value"><span style="font-size:15px; font-weight:bold; color:#f1c40f;">{row + 1:02d}</span></td>
                </tr>
                <tr>
                    <td class="label">{header_lang['status']}</td>
                    <td class="value">{"<span class='badge-lock'>🔒 核心鎖定保護中</span>" if is_locked else "<span class='badge-normal'>✔ 正常安全</span>" if self.current_lang_code == "zh-TW" else "<span class='badge-lock'>🔒 核心锁定保护中</span>" if is_locked else "<span class='badge-normal'>✔ 正常安全</span>"}</td>
                </tr>
                <tr>
                    <td class="label">{header_lang['mutex']}</td>
                    <td class="value">{f"<span style='color:#f39c12;font-weight:bold;'>⚔️ {mutex_group}</span>" if mutex_group else "<span style='color:#7f8c8d;'>無</span>" if self.current_lang_code == "zh-TW" else "<span style='color:#7f8c8d;'>无</span>"}</td>
                </tr>
                <tr>
                    <td class="label">{header_lang['title']}</td>
                    <td class="value" style="color:#5ade85; font-weight:bold;">{mdata.get('title', '未知')}</td>
                </tr>
                <tr>
                    <td class="label">{header_lang['update']}</td>
                    <td class="value" style="color:#a6acaf;">{mdata.get('time_updated', '未知')}</td>
                </tr>
                <tr>
                    <td class="label">{header_lang['path']}</td>
                    <td class="value" style="font-size:11px; color:#7f8c8d; word-break: break-all;">{clean_path}</td>
                </tr>
            </table>

            <div class="section-title">📋 {header_lang['trans']}</div>
            <div class="content-box">{smart_translation}</div>

            <div class="section-title orig">📂 {header_lang['orig']}</div>
            <div class="content-box english">{cleaned_original}</div>
        </body>
        </html>
        """
        self.txt_mod_details.setHtml(html_details)
        ...
        if full_img_path:
            ...
            self.lbl_mod_img.setText("【無預覽圖】" if self.current_lang_code == "zh-TW" else "【无预览图】")
    ...
    def scan_workshop_mods(self):
            ...
            for root, dirs, files in os.walk(self.mods_directory):
                for file in files:
                    if file.endswith(".pak"):
                        if "-windows" in file.lower() or "_windows" in file.lower():
                            continue
        ...
        if detected_new_mods:
            ...
            self.mod_list_widget.blockSignals(False)
            self.refresh_all_items_numbers()
            msg = f"🔍 發現並手動同步了 {len(detected_new_mods)} 個全新訂閱的模組！\n（已自動加入列表最下方，預設皆為「未啟用」狀態）" if self.current_lang_code == "zh-TW" else f"🔍 发现并手动同步了 {len(detected_new_mods)} 个全新订阅的模组！\n（已自动加入列表最下方，预设皆为「未启用」状态）"
            QMessageBox.information(self, "掃描成功" if self.current_lang_code == "zh-TW" else "扫描成功", msg)
        ...
        if scan_count == 0 and not detected_new_mods:
            msg = "所有模組皆已擁有本地說明快取，已自動跳過重複下載！" if self.current_lang_code == "zh-TW" else "所有模组皆已拥有本地说明缓存，已自动跳过重复下载！"
            QMessageBox.information(self, self.strings["success"], msg)

    def on_workshop_info_fetched(self, clean_path, result):
        if "-windows" in clean_path.lower() or "_windows" in clean_path.lower():
            return 
    ...
    def show_left_list_context_menu(self, pos):
        ...
        action_enable = QAction("啟用此模組" if self.current_lang_code == "zh-TW" else "启用此模组", self)
        action_disable = QAction("停用此模組" if self.current_lang_code == "zh-TW" else "停用此模组", self)
        ...
        is_locked = mdata.get("is_locked", False)
        action_lock = QAction("🔓 解除鎖定保護" if is_locked else "🔒 開啟鎖定保護" if self.current_lang_code == "zh-TW" else "🔓 解除锁定保护" if is_locked else "🔒 开启锁定保护", self)
        action_lock.triggered.connect(self.toggle_current_mod_lock)
        menu.addAction(action_lock)
        
        action_mutex = QAction("⚔️ 設定/修改 互斥群組" if self.current_lang_code == "zh-TW" else "⚔️ 设置/修改 互斥群组", self)
        action_mutex.triggered.connect(self.set_current_mod_mutex)
        menu.addAction(action_mutex)
        
        menu.addSeparator()
        tag_submenu = menu.addMenu("📌 快速套用全域標籤" if self.current_lang_code == "zh-TW" else "📌 快速套用全局标签")
        if self.global_tag_library:
            ...
            tag_submenu.addAction("（目前標籤庫為空）" if self.current_lang_code == "zh-TW" else "（目前标签库为空）").setEnabled(False)
    ...
    def export_mod_settings(self):
        ...
        if file_path:
            try:
                ...
                QMessageBox.information(self, self.strings["success"], "匯出防護與排序成功！" if self.current_lang_code == "zh-TW" else "导出防护与排序成功！")
            except Exception as e:
                QMessageBox.critical(self, self.strings["error"], f"匯出失敗:\n{str(e)}" if self.current_lang_code == "zh-TW" else f"导出失败:\n{str(e)}")
    ...
    def import_mod_settings(self):
        ...
        try:
            ...
            if "mod_data_map" in import_data:
                cleaned_import_map = self.clean_ghost_windows_paths(import_data["mod_data_map"])
                self.mod_data_map.update(cleaned_import_map)
            ...
            for x in order_list:
                if "-windows" in x["filename"].lower() or "_windows" in x["filename"].lower():
                    continue 
            ...
            for fname_lower, info in ui_items_map.items():
                if "-windows" in fname_lower or "_windows" in fname_lower:
                    continue 
                item = QListWidgetItem()
